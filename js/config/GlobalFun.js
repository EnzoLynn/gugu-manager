        ...
        alert: function(cfg, msg, fn, scope) {
            if (Ext.isString(cfg)) {
                cfg = {
                    ...
                    fn: fn,
                    scope: scope,
                    minWidth: this.minWidth < 330 ? 330 : this.minWidth
                };
            }
...
GlobalFun.setSeesionToken = function(sessiontoken, day) {
    ...
    Ext.util.Cookies.set("login_sessiontoken", sessiontoken,
        new Date(new Date().getTime() + (1000 * 60 * 60 * 24 * myday)));
};
...
GlobalFun.GetNextDate = function(day) {
    var today = new Date();
    var newDate = new Date();
    newDate.setTime(today.getTime() + (day * 24 * 3600 * 1000));

    return Ext.util.Format.date(newDate, 'Y-m-d');
}
...
//表格日期类型过滤菜单创建
GridManager.CreateGridFilterDateMenu = function(componet, param) {
    componet.menu.add(GridManager.GridFilterMenuCheckItemInit({
        group: param.group,
        checked: true,
        text: param.text,
            ...
            var win = Ext.create('Ext.window.Window', {
                ...
                buttons: [{
                    ...
                    handler: function() {
                        ...
                        if (dateStar || dateEnd) {
                            dateStar = Ext.util.Format.date(dateStar, 'Y-m-d');
                            dateEnd = Ext.util.Format.date(dateEnd, 'Y-m-d');
                            //加入.getProxy().extraParams
                            if (!panel.getStore().filterMap.containsKey(param.filterKey)) {
                                panel.getStore().filterMap.add(param.filterKey, dateStar + ',' + dateEnd);
                            } else {
                                panel.getStore().filterMap.replace(param.filterKey, dateStar + ',' + dateEnd);
                            }
                            searchFlag = true;
                        }
                        if (searchFlag) {
